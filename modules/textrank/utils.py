...
def scan_vocabulary(sents, tokenize=None, nWordSet = 1, min_count=2):
    """
    Arguments
    ---------
    sents : list of str
    ...
    """
    # print(w for sent in sents for w in tokenize(sent, nWordSet))
    counter = Counter(w for sent in sents for w in tokenize(sent, nWordSet))
    counter = {w:c for w,c in counter.items() if c >= min_count}
    idx_to_vocab = [w for w, _ in sorted(counter.items(), key=lambda x:-x[1])]
    vocab_to_idx = {vocab:idx for idx, vocab in enumerate(idx_to_vocab)}
    return idx_to_vocab, vocab_to_idx

def tokenize_sents(sents, tokenize, nWordSet = 1):
    """
    Arguments
    ---------
    sents : list of str
    ...
    """
    return [tokenize(sent, nWordSet) for sent in sents]
